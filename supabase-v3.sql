...
-- Compatible avec une base neuve OU la V1/V2 précédente.
...
-- Migration V1/V2 : autoriser le statut maintenance utilisé par la V3.
...
begin
  ...
  values(v_id,'system','Nairi','public','Votre dossier a bien été ouvert. Notre équipe va le qualifier puis revenir vers vous ici.');

  return v_ref;
end;
$$;

create or replace function public.track_case(p_reference text,p_phone text)
returns table(
  id uuid,
  reference text,
  kind text,
...
create or replace function public.case_public_messages(p_reference text,p_phone text)
returns table(
  id uuid,
  case_id uuid,
  author_type text,
...
create or replace function public.reply_case(p_reference text,p_phone text,p_body text)
returns boolean
language plpgsql
security definer
set search_path = public
...
begin
  ...
  select * into v_case
  from public.cases
  where upper(reference)=upper(trim(p_reference)) and phone=trim(p_phone)
  limit 1;
...
revoke all on function public.track_case(text,text) from public;
revoke all on function public.case_public_messages(text,text) from public;
revoke all on function public.reply_case(text,text,text) from public;
...
grant execute on function public.track_case(text,text) to anon, authenticated;
grant execute on function public.case_public_messages(text,text) to anon, authenticated;
grant execute on function public.reply_case(text,text,text) to anon, authenticated;
...
select 'Pounder','MTL','Poids lourd','available',
...
insert into public.fleet_vehicles(name,brand,category,status,image_url,description,active,sort_order)
select 'Speedo Express','Vapid','Utilitaire','available',
...
-- --------------------------------------------------------------------------
-- 14) OPTIONNEL : l'ancienne V1 Automotive peut rester en base.
-- La V3 ne lit plus vehicles / automotive_bookings / anciennes demandes.
